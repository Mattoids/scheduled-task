...
const groupQuery = reactive({ groupName: "" });
const filteredGroups = computed(() => {
  const keyword = groupQuery.groupName.trim();
  if (!keyword) return groupOptions.value;
  return groupOptions.value.filter((g) => g.groupName?.includes(keyword));
});
...
const saveRecipient = async () => {
  ...
  if (recipientFormId.value) {
    await updateRecipient(
      recipientFormId.value,
      recipientForm.value as EmailRecipient,
    );
  } else {
...
const openGroupForm = (row?: EmailRecipientGroup) => {
  ...
  groupForm.value = row
    ? { ...row }
    : { groupName: "", groupCode: "", description: "", status: 1 };
...
const saveGroup = async () => {
  ...
  if (groupFormId.value) {
    await updateGroup(
      groupFormId.value,
      groupForm.value as EmailRecipientGroup,
    );
  } else {
...
<template>
  <div class="page-card">
    <el-tabs v-model="activeTab">
      <el-tab-pane label="收件人" name="recipient">
        <BaseSearchForm
          ...
          @reset="handleRecipientReset"
        >
          <el-row>
            <el-col :span="6">
              <el-form-item label="收件人名称">
                <el-input
                  v-model="recipientQuery.recipientName"
                  placeholder="收件人名称"
                  clearable
                />
              </el-form-item>
            ...
            <el-col :span="6">
              <el-form-item label="所属分组">
                <el-select
                  v-model="recipientQuery.groupId"
                  placeholder="全部"
                  clearable
                >
                  <el-option
        ...
        <div class="table-toolbar">
          <el-button
            type="primary"
            v-permission="'email:create'"
            @click="openRecipientForm()"
            >新增收件人</el-button
          >
        </div>
          ...
          <el-table-column label="操作" width="160" fixed="right">
            <template #default="{ row }">
              <el-button
                link
                type="primary"
                v-permission="'email:edit'"
                @click="openRecipientForm(row)"
                >编辑</el-button
              >
              <el-button
                link
                type="danger"
                v-permission="'email:delete'"
                @click="handleDeleteRecipient(row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
    ...
    <el-dialog
      ...
      :title="recipientFormId ? '编辑收件人' : '新增收件人'"
      width="520px"
    >
