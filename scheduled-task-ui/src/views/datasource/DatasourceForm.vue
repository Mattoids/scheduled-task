...
import { createDatasource, getDatasource, updateDatasource, testDatasourceConfig } from '@/api/datasource'
...
const defaultJdbcParams = 'useSSL=false&serverTimezone=Asia/Shanghai&allowPublicKeyRetrieval=true'
...
const rules = {
  ...
  sshUsername: [{ required: true, message: '请输入 SSH 用户名', trigger: 'blur' }],
  sshPassword: [{
    validator: (_rule: any, value: any, callback: any) => {
      if (form.value.sshEnabled === 1 && sshAuthType.value === 'password' && !isEdit.value && !value) {
  ...
  sshPrivateKey: [{
    validator: (_rule: any, value: any, callback: any) => {
      if (form.value.sshEnabled === 1 && sshAuthType.value === 'key' && !isEdit.value && !value) {
...
const handleTest = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      const stageText = res.stage === 'SSH' ? 'SSH 连接失败' : res.stage === 'DATABASE' ? '数据库连接失败' : '连接失败'
  ...
  <el-dialog v-model="dialogVisible" :title="title" width="800px"
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="110px" v-loading="loading"
      ...
      <el-form-item label="JDBC 参数"
        >
        <el-input v-model="form.jdbcUrlParams" placeholder="如 useUnicode=true" />
      </el-form-item>
      ...
      <template v-if="form.sshEnabled === 1"
        ...
        <el-form-item v-if="sshAuthType === 'password'" label="SSH 密码"
          >
          <el-input v-model="form.sshPassword" type="password" placeholder="留空表示不修改" show-password />
        </el-form-item>
        <template v-if="sshAuthType === 'key'"
          >
          <el-form-item label="SSH 私钥"
            >
            <el-input v-model="form.sshPrivateKey" type="textarea" :rows="3" placeholder="留空表示不修改" />
          </el-form-item>
          <el-form-item label="私钥密码"
            >
            <el-input v-model="form.sshPassphrase" type="password" placeholder="留空表示不修改" show-password />
          </el-form-item>
        </template>
      </template>
    ...
    <ConnectionTestProgress :stages="testStages" :ssh-enabled="form.sshEnabled === 1" />
